## Landing Page Fix-1 / Fix-2 (16 Aug 2026)

- Landing Page live token price now renders with exactly five digits after the decimal (`0.XXXXX`).
- Landing Page disables browser scroll restoration and explicitly opens at the top after refresh/navigation restoration.
- Added the same floating Robinhood-lime back-to-top arrow used by CPB main, including mobile sizing and smooth scroll behavior.
- No market API, module routing, portal configuration, deployment, or builder logic was changed.
...
Chapter 15B will focus on a simplified end-user Builder experience. Infrastructure, payments, the planned **PORTAL** token, launch promotion, domain/hosting, and multi-tenant platform decisions remain final-product roadmap work and are maintained in **`PORTAL.md`**.
...
- The visible Builder version remains `v1.3.2-b`. `PORTAL.md` is the separate living final-product roadmap.
...
- Standalone Chapter 20B/21 checkpoint/fix-note files are consolidated into this README; `PORTAL.md` remains separate as the living final-product roadmap.
...
## CPB Simplified v1 RC Pass 10 — 15 Aug 2026
- Simplified CPB now uses a four-step flow: Project → Modules → Details → Deploy.
- Generate and Deploy are merged into one final Deploy experience while preserving the existing generator and protected GitHub + Render deployment engine underneath.
- Header branding now tells the CPB story with Build. → Launch. → Grow. appearing in sequence, followed by the Community Portal Builder shine sweep.
- Desktop empty-space artwork now uses visible, dimmed SVG illustrations for Build, Deploy, Grow, and Community rather than hidden background pseudo-elements.
- Adds animated Robinhood-lime and violet pre-footer energy lines.
- Footer retains its accepted footprint, removes the avatar ring border, keeps a subtle animated halo, and improves text readability on Robinhood lime.
- Simplified-mode dialogs, warnings, toasts, build-ready, and deployment-success feedback share one modern CPB visual system.

### CPB Simplified v1 RC Pass 11
Simplified-mode baseline and module selection now use a single visual language: empty square when off, solid accent-filled square when selected, with redundant checkmarks/badges removed.


## CPB RC Pass 12 — Consolidated Simplified UI polish
The Simplified CPB experience now uses neon-green baseline selection squares, accent-filled module selection squares, a clean black canvas, letter-by-letter Build. Launch. Grow. header animation, compact Deploy ending, tightened pre-footer energy lines, and final dark-blue footer typography. The existing generator/deployment engine remains unchanged.


## CPB Simplified v1 RC Pass 12.4 — Chapter 22C handoff
- Final pre-test hotfix separates EVM address-format validation from live market/discovery checks.
- Valid Token/NFT CA syntax now drives the input border and format status green immediately; downstream market/discovery warnings remain independent.
- Source-of-truth handoff for **Chapter-22C Tests on CPB New UI**.

## Chapter 22C — Test/Fix Cycle 1 (15 Aug 2026)

The Simplified CPB UI received three focused presentation fixes discovered during mobile testing: the four-step journey no longer clips horizontally on phones, the Robinhood-lime footer is compact and viewport-safe on mobile, and the `Build. Launch. Grow.` sequence now adds a post-reveal shine sweep and replays every 5 seconds. These are isolated Simplified-mode changes; Builder Mode and portal generation/deployment behavior remain unchanged.
...
- Retired and removed the former `SHELL Token_Readme.md` roadmap.
- Added `PORTAL.md` as the living final-product roadmap and future-phase source of truth. The planned ecosystem token is now **PORTAL**, not SHELL.
- Recorded the current commercial direction that paid CPB modules should support **PORTAL or ETH**, with exact pricing/token economics intentionally deferred until the payment layer is designed and implemented.
- Added the **First 3 Projects Free** global launch promotion specification: the first three distinct projects to complete a qualifying first real portal creation/deployment receive the full portal free; claims must be tracked atomically server-side by counter + project IDs, and previews/failures/rebuilds/redeployments do not consume additional slots.
- Added final-product decision phases for the CPB domain, production architecture, and **Render paid vs alternative production hosting**, while explicitly retaining the current Render integration until an alternative passes equivalent end-to-end deployment testing.
- Migrated still-relevant future architecture, module, infrastructure, mobile-readiness, and Load/Save final-product notes into `PORTAL.md`.
- Documentation-only pass: no CPB runtime, generator, deployment, routing, validation, or Builder Mode behavior changed.
...
## Chapter 22C — Pass 12.13: first generated portal UI modernization

- Modernized `01_Landing-Page` as the first generated CPB-era portal surface.
- Preserved all existing live-market, contract-copy, module-link, configuration, footer-runtime and server bindings.
- Replaced the older terminal presentation with the accepted CPB visual language: black canvas, rounded dark product panels, lime/aqua/violet accents, modern status chips, modern buttons, card-based live data and responsive mobile layouts.
- This page is intentionally the first visual reference for later generated-terminal modernization; other terminal modules remain unchanged while Chapter 22C functional testing begins.
- Added `test-chapter22c-landing-modernization.js` to protect both the new presentation hooks and the existing functional IDs/runtime bindings.
...
## Chapter 22C — Test-1 UI + Deployment Polish (Pass 12.15)

CPBT1/CPBT2 testing produced the first full generated-portal and deployment-modal polish bundle. The generated Landing Page now presents **<Project Name> Community Portal** with a centered logo, neon-green project title, stronger orange/violet/aqua/lime accents, cleaner status/readiness surfaces, more readable tool cards, and the CPB-style footer with two energy lines. Preview Portal now mirrors this modernized portal direction rather than the legacy terminal preview.

The Portal-ready deployment modal now reads **Portal ready to deploy**, uses Robinhood Yellow for the project identity, CPB primary CTA styling for Deploy/Confirm, rounded confirmation UI, the simplified exact phrase **CONFIRM DEPLOY**, product-facing pre-deployment wording without infrastructure vendor names, and a guarded Close action while deployment is active. Public verification recognizes both Community Terminal and Community Portal landing identities.


## Chapter 22C — Temporary QA Transfer Field (Pass 12.16)

- Added one isolated, temporary Simplified V1 QA field containing **EXPORT TEST CONFIG** and **IMPORT TEST CONFIG**.
- The field is only a thin UI alias over the already-preserved Builder Mode import/export functions; project schema, save/load behavior, generation, deployment, and Builder Mode are unchanged.
- The block is explicitly marked `TEMPORARY CHAPTER 22C QA FIELD` in HTML/CSS/JS so it can be removed cleanly after testing.
...
- Saved-project Load now carries Public Acceptance forward automatically for continued QA/redeployment.
- Deployment modal LIVE PORTAL card gets a small top spacing refinement.

## Chapter 22C — Pass 12.21 (15 Aug 2026)
Polish batch after successful loaded-project deployment test: pre-success live URL is display-only until final success, success modal hierarchy is refined, CPB footer typography/avatar geometry is corrected, and generated Landing Page spacing/header/Market Snapshot/module/footer presentation is compacted while preserving runtime behavior.


### Chapter 22C — Pass 12.22 (deployed portal div-by-div review)
- Tightened generated portal header and centered project logo artwork.
- Renamed LIVE DATA to LIVE MARKET DATA; real LIVE indicators now use neon green.
- Portal Status now renders only READY module rows in one vertically aligned stack.
- Quick Access and Available Modules remain separate but now use equal-height desktop cards.
- Canonicalized the exact CPB animated two-line pre-footer treatment across generated portal pages.
- Introduced the new 25/50/25 footer: dark-blue left identity panel with lime text, Robinhood-lime center band, dark-blue right creator panel with compact original-scale avatar, bright-white creator text and clickable ice-blue X handle.
...
- Removed the Landing Page hero border and placed the canonical animated lime/violet double-divider directly beneath the header.
- Fixed CSS specificity so Price, Market Cap, Holders, and 24H Volume render Orange in the deployed portal; project/social links now render in the warm amber accent.
- Removed Quick Access as a separate card. `Your Portal Tools` is now the single full-width module section, with each module receiving a direct colored launch button.
- Cache-busted Landing Page CSS/JS so the visual patch is picked up immediately after deploy.
- Preserved the compact canonical yellow footer and all successful loaded-project/deployment behavior.


## Chapter 22C — Pass 12.29 (15 Aug 2026)
- Final Landing Page header cleanup: removed the remaining hero background/gradient layer so the header sits directly on the page canvas.
- Hardened fresh Render deployment readiness with repeated uncached public acceptance checks and a final settle verification before CPB exposes the live success state.
- Preserved Pass 12.28 short-viewport deployment modal/button accessibility.

## Whale Activity Tracker — NEW UI Pass 1 (16 Aug 2026)
- First module-page conversion to the accepted Landing Page visual system.
- Shared Landing header/footer styling reused; module subtitle is bright-orange `Whale Activity Tracker` directly above ONLINE.
- Preserved whale market data, Top-30/12h commands, wallet/rank lookups, activity tables, routing and API hooks.
- Added Landing-standard animated header divider, responsive portal cards, refresh-to-top behavior and shared back-to-top control.
- Focused Whale UI, generator and first-load reliability checks pass.

## Whale Activity Tracker — NEW UI Pass 2 (16 Aug 2026)
- Merged `Quick Commands` and `Available Commands` into one full-width Landing-style command hub while preserving all command hooks and input-prefill behavior.
- Enforced the accepted accent rule: the merged Fast Analysis title follows its aqua left border; the nested Available Commands subsection follows its violet left border.
- Restored `24H PRICE CHANGE` with ▲/▼ direction and percentage beside Price on both Landing Page and Whale Activity Tracker; Landing `/api/price` now forwards DexScreener `priceChange.h24`.
- Kept the accepted five-decimal Landing/Whales USD price format and added narrow-mobile wrapping for price + momentum without page overflow.
- Added dependency-free adaptive visual summaries inside the existing Whale command-output card for holdings, Top-30 activity mix, rank movers, net-flow commands and statistics; detailed text/tables remain directly below.
- Added responsive chart/tile rules so visuals stack safely on narrow mobile screens; wide data tables retain their existing internal horizontal-scroll containment.
- Whale Pass 1 regression, Whale Pass 2 focused regression, Landing Fix-1/Fix-2, CPB generator and first-load reliability checks pass.

### 2026-08-16 — Meme Intelligence Portal / Whale Activity Tracker refinement
- Updated Meme Intelligence to the accepted Landing/Whales portal UI language while preserving command/data behavior.
- Whale mobile market price change now renders as a compact one-line `PRICE CHANGE ▲/▼ %` directly below the price.
- Command execution on Whale Activity Tracker and Meme Intelligence Portal now returns the viewport to the first prompt/output line for the command after rendering.
- Whale activity tables now show a contextual Holder Rank note only when `N/A (Contract)`, `N/A (Unranked)`, or `N/A (No Balance)` statuses are present.
...
## Portal UI update — Community Timeline
- Canonical module header rules are now shared through the common portal stylesheet; only the bright-orange module subtitle varies by page.
- Community Timeline received the accepted Portal UI treatment and automatic starter milestones for portal creation/module activation plus configured NFT schedule milestones.
- CPB deployment-success status line received a small alignment/readability refinement.

### 16 Aug 2026 — NFT Portal NEW UI Pass 1
- NFT Countdown and NFT Collection Portal updated to the accepted CPB portal design language while preserving countdown, phase, mint, OpenSea, wallet, sales and analytics logic.
- NFT pages use the canonical shared header contract: Community Portal title, exact bright-orange NFT module subtitle above ONLINE, animated lime/violet divider, and compact shared footer at generation time.
- Added refresh-to-top and shared back-to-top control to both NFT surfaces.
- User-facing NFT terminology now uses Portal rather than Terminal; internal route/class identifiers remain unchanged for compatibility.

## Chapter 22C — NFT Portal NEW UI Pass 2 (16 Aug 2026)
- Standardized NFT Countdown and Collection Portal headers to the canonical shared portal header: yellow main title, bright-orange module subtitle, shared divider/footer behavior.
- Refined mint lifecycle presentation: `MINT COMPLETE` (neon green), `MINT LIVE` (bright orange), and `Mint has not started yet.` (bright white).
- Completed mint state now retains a compact one-line phase schedule/price/wallet-limit summary above the post-mint tracking line for single- and multi-phase configurations.
- Replaced countdown boot/status-log presentation with modern access cards.
- Removed public NFT collection diagnostic boot log and generic project additional links from the NFT Collection Portal.
- Merged Quick Commands and Available Commands into one modern Tools card while preserving all command hooks.
- Enlarged and widened the Market Update / NFT Sales panel, made it page-height with an internally scrolling sales feed, and made the NFT footer span the full two-column layout.
- Strengthened the past-schedule warning acknowledgement so an unchanged past mint schedule warns once and does not loop.
- Focused generated single-phase/multi-phase, lifecycle, first-load, Landing, Whale, Intel, Pulse, Timeline, and generator regression checks pass.


### Chapter 22C — NFT final acceptance refinement
- Locked NFT headers to the same canonical Community Portal title color/structure used by the accepted Landing and module pages; only the bright-orange module subtitle varies.
- Generated portal refresh/navigation restore now settles at the top after initialization instead of being pulled toward Market Data by early rendering.
- Reduced the completed-mint hero scale, kept phase history, aligned desktop wallet input/action, removed residual bracketed LIVE/HOLDERS presentation, and let Market Update stretch with the full NFT page.

## 16 Aug 2026 — NFT Pass 3B acceptance lock

- Preserved cache-buster query strings when CPB prefixes generated module asset routes, preventing stale NFT CSS/JS after redeploy.
- Canonical NFT header title remains Robinhood lime; module subtitle remains bright orange.
- NFT Market Update sidebar stretches with the full desktop content row; only its inner sales list scrolls.
- Shared generated footer now attaches to the full NFT two-column workspace and spans beneath both main content and Market Update.
- Final deployment-success CLOSE action now uses the established CPB red close-button treatment.

## 16 Aug 2026 — NFT Pass 3C multi-phase test lock
- Locked the NFT Community Portal main header title to the explicit canonical Robinhood lime `#ccff00` in both single- and multi-phase templates.
- Renamed the sales-sidebar heading to `NFT MARKET UPDATE` at runtime and centered it in the panel.
- Bumped NFT asset cache versions to `cpb-nft-pass3c` so deployed single/multi-phase tests cannot reuse the previous header/label assets.


### NFT NEW UI final acceptance pass 4
- Locked canonical NFT header color and centered NFT MARKET UPDATE assets with a fresh cache version.
- Multi-phase countdown now uses the shared portal width/spacing rhythm, purple rounded phase cards, correct MINT LIVE/MINT COMPLETE page state, and no project X access card.
- Single- and multi-phase countdown action buttons now use the modern CPB portal UI.
- Mobile NFT Collection layout is explicitly ordered: main content → NFT MARKET UPDATE → footer.
- CPB NFT mint confirmation warning is larger/red, the pending confirmation button is red, and the review modal uses green Confirm + yellow Change/Edit.
...
## Clean baseline finalization
- Removed the temporary guided-mode QA project transfer strip from the CPB main page.
- Preserved the real Builder Mode project import/export controls.
- Added a one-time browser migration that clears legacy CPB saved-project, workspace, deployment, and recovery test data so this release starts from a blank workspace.
- Accepted portal/NFT UI and deployment logic remain unchanged.

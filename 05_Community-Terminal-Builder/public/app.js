...
const CLEAN_BASELINE_KEY="cpb.clean-baseline.final6";
function applyCleanBaselineMigration(){
  try{
    if(localStorage.getItem(CLEAN_BASELINE_KEY)==="done")return;
    for(const key of [STORAGE_KEY,SETTINGS_KEY,DEPLOYMENT_KEY,RECOVERY_KEY])localStorage.removeItem(key);
    localStorage.setItem(CLEAN_BASELINE_KEY,"done");
  }catch{}
}
applyCleanBaselineMigration();
